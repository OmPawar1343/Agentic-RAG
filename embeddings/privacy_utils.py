...
RE_SALARY  = re.compile(r"\b(salary|ctc|compensation|wage)\b", re.I)
...
def _detect_output_pii(answer_text: str) -> set[str]:
    ...
    if RE_PHONE.search(s):   shown.add("Phone")
    if RE_DOB.search(s):     shown.add("DOB")
...
def _risk_score_guard_aware(requested_sensitive: list[str], role_verified: bool,
    ...
    base = {
        ...
        "Salary":55,"DOB":45,"Address":35,"Location":35,"Phone":25,"Email":20,"EmpID":10,"Gender":10
    }
...
def privacy_meter_report(
    ...
    req = [str(x) for x in (requested_sensitive_cols or []) if str(x).strip()]
    ...
    csv_row = {
        ...
        "requested_sensitive": ", ".join(sorted(set(req))) if req else "-",
    ...
    json_obj = {
        ...
        "requested_sensitive": sorted(list(set(req))) if req else [],
    ...
    if METER_LOG_FULL:
        # Why: Optional full payloads for deep-dive audits (turn off in prod).
        json_obj["question_full"] = question
        json_obj["answer_full"] = answer_text
    ...
    if METER_COMPACT:
        rows = [
            ...
            f"| Stage | {stage_disp} |",
            f"| Risk | {score} ({level}) |",
            f"| Input flags | {_flags_str(flags)} |",
        ]
        if req:
            rows.append(f"| Requested sensitive | {', '.join(sorted(set(req)))} |")
        if gates_fired != "-":
